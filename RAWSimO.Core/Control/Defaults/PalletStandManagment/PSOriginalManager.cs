...
{
    /// <summary>
    /// This class is the original one. Thus, uses only first Waypoint.
    /// </summary>
    public class PSOriginalManager : PalletStandManager
    {
        /// <summary>
        /// Constructor which sets Instance.
        /// </summary>
        /// <param name="instance"></param>
        public PSOriginalManager(Instance instance)
        {
            Instance = instance;
        }
        ...
        public override Waypoint GetOutputPalletStandWaypoint(BotNormal bot)
        {
            ...
            /*foreach (var ps in Instance.OutputPalletStands)
            {
                Console.WriteLine($" {ps.ID}:  AssignedOrders - {ps.AssignedOrders.Count()}  QueuedOrders - {ps.QueuedOrders.Count()}");    
            }*/
